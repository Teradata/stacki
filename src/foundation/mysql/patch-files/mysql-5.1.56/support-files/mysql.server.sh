...
extra_args="-c $basedir/etc/my.cnf" ## Use Stack Config ONLY
...
case "$mode" in
  'start')
    ...
    then
      ...
      then
        touch /var/lock/subsys/foundation-mysqlmanager
      fi
      exit $return_value
    ...
    then
      ...
      if test -w /var/lock/subsys
      then
        touch /var/lock/subsys/foundation-mysql
      fi
      exit $return_value
    else
  ...
  'stop')
    ...
    lock_dir=/var/lock/subsys/foundation-mysqlmanager

    # If the manager pid_file doesn't exist, try the server's
    if test ! -s "$pid_file"
    then
      pid_file=$server_pid_file
      lock_dir=/var/lock/subsys/foundation-mysql
    fi
    ...
    if test -s "$server_pid_file" ; then 
      ...
      if test -z $mysqld_pid ; then 
        if test "$use_mysqld_safe" = "0" ; then 
          lockfile=/var/lock/subsys/foundation-mysqlmanager
        else
          lockfile=/var/lock/subsys/foundation-mysql
        fi 
