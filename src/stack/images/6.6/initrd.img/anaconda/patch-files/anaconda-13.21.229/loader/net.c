...
int wait_for_iface_activation(char *ifname, int timeout) {
	...
	if (device != NULL) {
		...
		g_main_loop_unref(loop);
                logMessage(INFO, "STACKI:net.c:1:client 0x%lx",
    ...
    g_main_loop_unref(loop);
    logMessage(INFO, "STACKI:net.c:2:client 0x%lx",
        (unsigned long int)client);
    g_object_unref(client);
    ...
    if (device == NULL) {
        logMessage(ERROR, "%s (%d): network device %s not found",
                   __func__, __LINE__, ifname);
        logMessage(INFO, "STACKI:net.c:3:client 0x%lx",
            (unsigned long int)client);
#ifndef	STACKI
        /*
...
int wait_for_iface_disconnection(char *ifname) {
    ...
    if (device == NULL) {
        logMessage(ERROR, "%s (%d): network device %s not found",
                   __func__, __LINE__, ifname);
        logMessage(INFO, "STACKI:net.c:4:client 0x%lx",
            (unsigned long int)client);
        g_object_unref(client);
        return 3;
    }
    ...
    while (count < 5) {
        ...
        if (state == NM_DEVICE_STATE_DISCONNECTED) {
            ...
            res_init();
            g_main_loop_unref(loop);
            logMessage(INFO, "STACKI:net.c:5:client 0x%lx",
                (unsigned long int)client);
            g_object_unref(client);
            return 0;
        }
    ...
    g_main_loop_unref(loop);
    logMessage(INFO, "STACKI:net.c:6:client 0x%lx",
        (unsigned long int)client);
    g_object_unref(client);
    return 3;
}
