...
def doInstall(storage, payload, ksdata, instClass):
    ...
    if 'boss' in args:
        ...
        bossenv['DISPLAY'] = ':1'
	with progress_report(_("Downloading Pallets")):
		s = subprocess.Popen(
			['/opt/stack/bin/boss_download_pallets.py'],
            		env = bossenv)
       		rc = s.wait()
       	log.debug('STACKI: Downloading pallets: exited with code %d'  % rc)

	# Reset links for lighttpd
	cmd = 'rm -rf /install ; '
	cmd += 'ln -s /mnt/sysimage/export/stack /install'
	os.system(cmd)
