...
def frontend_ssh_to_backends(host, backend):
    ...
    command_returns = []
    cmd = host.run("sudo -i ssh {} cat /etc/hostname".format(backend))
    assert cmd.stdout.strip() == "{}.localdomain".format(backend)
    assert (cmd.rc == 0)
...
def setup_partitioning(host, backend, test_scenario):
    ...
    if "tdc_" in test_scenario[0]:
        partition_config = [
            {'Name': '{}'.format(backend), 'Device': 'sda', 'Mountpoint': '', 'Type': '',
    ...
    # with open("{}_partition.csv".format(backend), 'w+') as csvfile:
    ...
    for each in fake_file.getvalue().splitlines():
        my_string += "\\n" + each
    cmd = host.run('sudo -i echo -e "%s"' % my_string + " > /home/vagrant/{}_partition.csv".format(backend))
    assert cmd.rc == 0
    cmd = host.run("sudo -i stack remove storage partition {}".format(backend))
    assert cmd.rc == 0
    cmd = host.run("sudo -i stack load storage partition file=/home/vagrant/{}_partition.csv".format(backend))
    if test_scenario[0] != "xfs":
        assert cmd.rc == 0
    cmd = host.run("sudo -i stack set host attr {} attr=disklabel value=".format(backend) + test_scenario[2])
    assert cmd.rc == 0
    return partition_config


def setup_os(host, backend, test_scenario, nukedisk=True):
    """Setup the os for installing by applying the box and proper installaction"""
    if test_scenario[1] == "SLES_11":
        cmd = host.run("sudo -i stack set host box {} box=sles11".format(backend))
        assert cmd.rc == 0
        cmd = host.run("sudo -i stack set host installaction {} action='install sles 11.3'".format(backend))
        assert cmd.rc == 0
    elif test_scenario[1] == "DEFAULT":
        cmd = host.run("sudo -i stack set host box {} box=default".format(backend))
        assert cmd.rc == 0
        cmd = host.run("sudo -i stack set host installaction {} action='default'".format(backend))
        assert cmd.rc == 0
    cmd = host.run("sudo -i stack set host boot {} action=install nukedisks=".format(backend) + str(nukedisk))
    assert cmd.rc == 0
...
def wait_for_install(host, backend):
    ...
    while "install" in cmd.stdout:
        ...
        print(cmd.stdout)
        host.run("i=0;while sleep 60; do sudo -i ssh {} 'exit 0' && break; i=$((i+1)); "
                 "if [[ $i -gt 20 ]]; then exit 1; fi; done".format(backend))
        i += 1
        if i > 2:
    ...
    cmd = host.run("sudo -i ssh {} 'exit 0'".format(backend))
    i = 0
...
def check_backend_reinstall(host, backend, nukedisk=True, first_install=True):
    ...
    if test_scenario is None or (test_scenario[1] == "SLES_11" and not TEST_SLES_11):
        ...
        setup_os(host, backend, test_scenario, nukedisk)
        # Create data files if backend is running
        if powered_on(host, backend) and not first_install:
            for each in partitions:
...
def test_nukedisk_false(host, backend):
    ...
    run_throughs = (len(BACKEND_TUPLE) + len(TEST_SCENARIOS) - 1 / len(BACKEND_TUPLE))
    global RUN_NUM
...
./set-up.sh /Users/ts186032/Documents/GitHub/stacki-iso-builder/stacki-5.0_20180315_8a89e9a-sles12.x86_64.disk1.iso
# Wait a long time..
