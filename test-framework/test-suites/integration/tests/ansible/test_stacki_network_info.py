class TestStackiNetworkInfo:
    ...
    def test_no_network_name(self, run_ansible_module):
        result = run_ansible_module("stack_network_info")

        assert result.status == "SUCCESS"
        assert result.data["changed"] is False
        assert len(result.data["networks"]) == 9

    def test_network_name(self, run_ansible_module):
        network_name = "primary"
        result = run_ansible_module("stack_network_info", name=network_name)

        assert result.status == "SUCCESS"
        assert result.data["changed"] is False
        assert len(result.data["networks"]) == 1
        assert result.data["networks"][0]["name"] == network_name
