...
class TestRemoveEnvironmentFirewall:
	def test_remove_environment_firewall_no_args(self, host):
	...
	def test_remove_environment_firewall_invalid(self, host):
	...
	def test_remove_environment_firewall_no_rulename(self, host, add_environment):
	...
	def test_remove_environment_firewall_invalid_rulename(self, host, add_environment):
		result = host.run('stack remove environment firewall test rulename=test')
		assert result.rc == 255
		assert result.stderr == 'error - firewall rule test does not exist for environment test\n'

	def test_remove_environment_firewall_one_arg(self, host, add_environment):
		# Add a firewall rule
		result = host.run(
	...
	def test_remove_environment_firewall_multiple_args(self, host, add_environment):
