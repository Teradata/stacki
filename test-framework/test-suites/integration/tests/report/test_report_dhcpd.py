...
class TestReportDhcpd:
	def test_duplicate_interface(self, host):
		result = host.run('stack add host interface frontend-0-0 interface=eth2 mac=00:11:22:33:44:55 ip=1.2.3.4 network=private')
		assert result.rc == 0
	...
	def test_invalid_channel(self, host):
		result = host.run('stack add host interface frontend-0-0 interface=eth2 mac=00:11:22:33:44:55 channel=alvin')
		assert result.rc == 0

		# interface with invalid channel should be excluded from report
		result = host.run('stack report dhcpd')
		assert result.rc == 0
		assert 'eth2' not in result.stdout
