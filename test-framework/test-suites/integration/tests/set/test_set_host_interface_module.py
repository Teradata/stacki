...
class TestSetHostInterfaceModule:
	...
	def test_invalid_interface(self, host):
		result = host.run('stack set host interface module frontend-0-0 interface=eth9 module=test')
		assert result.rc == 255
		assert result.stderr == 'error - interface "eth9" does not exist for host "frontend-0-0"\n'

	def test_invalid_mac(self, host):
		result = host.run('stack set host interface module frontend-0-0 mac=00:11:22:33:44:55 module=test')
		assert result.rc == 255
		assert result.stderr == 'error - mac "00:11:22:33:44:55" does not exist for host "frontend-0-0"\n'

	def test_invalid_network(self, host):
		result = host.run('stack set host interface module frontend-0-0 network=test module=test')
		assert result.rc == 255
	...
	def test_by_network(self, host, add_host, add_network):
		...
		result = host.run('stack set host interface module backend-0-0 network=test module=test_module')
		assert result.rc == 0
	...
	def test_by_interface(self, host, add_host_with_interface):
		# Set the host interface module
		result = host.run('stack set host interface module backend-0-0 interface=eth0 module=test_module')
		assert result.rc == 0
		...
		result = host.run('stack set host interface module backend-0-0 interface=eth0 module=null')
		assert result.rc == 0
	...
	def test_by_mac(self, host, add_host):
		...
		# Set the host interface module
		result = host.run('stack set host interface module backend-0-0 mac=00:11:22:33:44:55 module=test_module')
		assert result.rc == 0
