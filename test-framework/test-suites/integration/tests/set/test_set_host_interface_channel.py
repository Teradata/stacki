...
class TestSetHostInterfaceChannel:
	...
	def test_invalid_network(self, host):
		result = host.run('stack set host interface channel frontend-0-0 network=test channel=1')
		assert result.rc == 255
	...
	def test_by_network(self, host, add_host, add_network):
		...
		result = host.run('stack set host interface channel backend-0-0 network=test channel=1')
		assert result.rc == 0
	...
	def test_by_interface(self, host, add_host_with_interface):
		...
		result = host.run('stack set host interface channel backend-0-0 interface=eth0 channel=null')
		assert result.rc == 0
