...
class TestSetHostName:
	def test_set_host_name_no_hosts(self, host):
	...
	def test_set_host_name_no_matching_hosts(self, host):
	...
	def test_set_host_name_no_parameters(self, host):
	...
	def test_set_host_name_already_used(self, host, add_host):
		result = host.run('stack set host name backend-0-0 name=frontend-0-0')
		assert result.rc == 255
		assert result.stderr == 'error - name already exists\n'

	def test_set_host_name_single_host(self, host, add_host, host_os):
		# Set the host name
	...
	def test_set_host_name_multiple_hosts(self, host, add_host):
