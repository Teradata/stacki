...
class TestListSwitchPartitionMember:
	...
	def test_list_by_name(self, host, add_ib_switch, add_ib_switch_partition, add_host_with_interface, test_file):
		result = host.run(f'stack add host interface backend-0-0 interface=ib0 mac=00:00:00:00:00:00:00:00')
		assert result.rc == 0
		...
		result = host.run(f'stack add switch partition member switch-0-0 name=default guid=00:00:00:00:00:00:00:00')
		assert result.rc == 0
		...
		result = host.run(f'stack add switch partition member switch-0-0 name=aaa guid=00:00:00:00:00:00:00:00')
		assert result.rc == 0

		result = host.run(f'stack list switch partition member switch-0-0 name=aaa output-format=json')
		assert result.rc == 0
		with open(test_file(f'add/add_nondefault_member_output.json')) as output:
			assert json.loads(result.stdout) == json.loads(output.read())

		# valid names should return nothing, and not error
		result = host.run(f'stack list switch partition member switch-0-0 name=bbb output-format=json')
		assert result.rc == 0
		assert result.stdout.strip() == ''
		assert result.stderr.strip() == ''

		# invalid names should return an error
		result = host.run(f'stack list switch partition member switch-0-0 name=badname output-format=json')
		assert result.rc != 0
		assert result.stdout.strip() == ''
		assert result.stderr.strip() != ''

	def test_list_everything(self, host, add_ib_switch, add_ib_switch_partition, add_host_with_interface, test_file):
		...
		with open(test_file('add/add_multiple_partitions_multiple_members_output.json')) as output:
			expected_output = output.read()
