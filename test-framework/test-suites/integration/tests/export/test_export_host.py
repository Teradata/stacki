...
class TestExportHost:

	"""
	Test that exporting the host data works properly by adding some host information
	then exporting it and checking that it is valid
	"""

	def test_export_host(self, host):

		# first lets add a host so we know what to look for in the export
		...
		for host in exported_data['host']:
