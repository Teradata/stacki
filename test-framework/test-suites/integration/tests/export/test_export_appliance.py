...
class TestExportAppliance:

	"""
	Test that exporting the appliance data works properly by adding some appliance information
	then exporting it and checking that it is valid
	"""

	def test_export_appliance(self, host):

		# first lets add some appliance info so we know what to look for in the export
...
#		assert results.rc == 0
		...
		# export our appliance information
		...
		for appliance in exported_data['appliance']:
