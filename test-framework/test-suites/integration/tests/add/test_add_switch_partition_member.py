...
class TestAddSwitchPartitionMember:
	...
	def test_cannot_duplicate_names_that_resolve_same(self, host, add_ib_switch, add_host_with_interface, test_file):
		...
		result = host.run(f'stack add switch partition member switch-0-0 name={same_parts[0]} guid=00:00:00:00:00:00:00:00')
		assert result.rc == 0
		for partition_name in same_parts[1:]:
			result = host.run(f'stack add switch partition member switch-0-0 name={partition_name} guid=00:00:00:00:00:00:00:00')
			assert result.rc != 0
			result = host.run('stack list switch partition member switch-0-0 output-format=json')
			assert result.rc == 0
			assert json.loads(result.stdout) == json.loads(expected_output)
