...
class Command(stack.commands.add.command):
	"""
	Add storage partition configuration for an host.
	...
	<example cmd='add host storage partition backend-0-0 device=sda mountpoint=/var
		size=50 type=ext4'>
	Creates a ext4 partition on device sda with mountpoints /var.
	for the host 'backend-0-0'
	</example>

	<example cmd='add host storage partition backend-0-0 device=sda partid=1
