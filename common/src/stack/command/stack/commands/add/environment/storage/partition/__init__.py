...
class Command(stack.commands.add.command):
	"""
	Add storage partition configuration for an environment.
	...
	<example cmd='add environment storage partition master_node device=sda mountpoint=/var
		size=50 type=ext4'>
	Creates a ext4 partition on device sda with mountpoints /var
	for the environment 'master_node'
	</example>

	<example cmd='add environment storage partition master_node device=sda partid=1
	...
	def run(self, params, args):
		self.addText(self.command('add.storage.partition', self._argv + ['scope=environment']))
		return self.rc
