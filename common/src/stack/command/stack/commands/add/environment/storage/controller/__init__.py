...
class Command(stack.commands.add.command):
	...
	<example cmd='add environment storage controller master_node slot=1 raidlevel=0 arrayid=1'>
	The disk in slot 1 on backend-0-0 should be a RAID 0 disk, for the environment 'master_node'
	</example>

	<example cmd='add environment storage controller master_node slot=2,3,4,5,6 raidlevel=6 hotspare=7,8 arrayid=2'>
	The disks in slots 2-6 on backend-0-0 should be a RAID 6 with two
	hotspares associated with the array in slots 7 and 8, for the environment 'master_node'
	</example>
	"""
