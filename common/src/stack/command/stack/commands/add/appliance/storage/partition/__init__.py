...
class Command(stack.commands.add.command):
	...
	<example cmd='add appliance storage partition backend device=sda mountpoint=/var
		size=50 type=ext4'>
	Creates a ext4 partition on device sda with mountpoints /var
	for the appliance 'backend'
	</example>

	<example cmd='add appliance storage partition backend device=sda partid=1
