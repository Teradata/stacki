...
class Command(PalletArgumentProcessor, command):
	...
	def perform_checksum(self, pallet_arg, iso_name):
		if self.checksum.get(pallet_arg):
			checksum_type, checksum_value = self.checksum.get(pallet_arg)
			self.notify(f'Checking {iso_name} matches {checksum_type}={checksum_value}...')
			checksum_input = f'{checksum_value} {iso_name}\n'
			proc = self._exec(
			...
			if proc.returncode != 0:
				msg = f'Checksum for {iso_name} does not match given checksum ({checksum_type}:{checksum_value}).'
	...
	def run(self, params, args):
		...
		if checksum:
			...
			if bad_paths:
				msg = f"One or more paths was specified with a checksum that is a directory. Invalid paths are: " \
					  f"{', '.join(bad_paths)}"
				raise UsageError(self, msg)

			if len(checksum.split(',')) != len(args):
				msg = f"Checksum is required for each pallet."
				raise UsageError(self, msg)
			...
			for i, pallet in enumerate(args):
				try:
					...
					msg = 'You must supply a checksum in the format of <type>:<value>'
					raise UsageError(self, msg)
			...
			if len(invalid_checksums) > 0:
				msg = f"Invalid checksum type(s) given: {', '.join(invalid_checksums)}. " \
