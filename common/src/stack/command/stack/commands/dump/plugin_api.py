...
class Plugin(stack.commands.Plugin):
	...
	def run(self, args):
		...
		api_group_data = self.owner.command('list.api.group', [ 'output-format=json' ])
		api_group_prep = []
		if api_group_data:
			# group['USERS'] is a ' ' delimeted string we need to turn into a list
			api_group_prep = {group['GROUP']: {'users': [user for user in group['USERS'].split()]} for group in json.loads(api_group_data)}
			# now that we have all of the groups, get each groups permission data
			for group, data in api_group_prep.items():
				api_group_perm_data = self.owner.command('list.api.group.perms', [ group, 'output-format=json' ])
				if api_group_perm_data:
					data['permissions'] = [item['Command'] for item in json.loads(api_group_perm_data)]
				else:
					data['permissions'] = []

		# grab all of the api users
		api_user_data = self.owner.command('list.api.user', [ 'output-format=json' ])
		api_user_prep = []
		if api_user_data:
			api_user_prep = json.loads(api_user_data)
			...
			for user in api_user_prep:
				...
				api_user_perm_data = self.owner.command('list.api.user.perms', [ user['username'], 'output-format=json' ])
				if api_user_perm_data:
					...
					user['permissions'] = [item['Command'] for item in json.loads(api_user_perm_data) if item['Source'] != 'G']
				else:
					user['permissions'] = []

		# grab all of the blacklisted api commands
		blacklist_command_data = self.owner.command('list.api.blacklist.command', [ 'output-format=json' ])
		blacklist_command_prep = []
		if blacklist_command_data:
			blacklist_command_prep = [command['Command'] for command in json.loads(blacklist_command_data)]
