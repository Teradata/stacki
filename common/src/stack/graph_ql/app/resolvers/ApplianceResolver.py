...
def resolve_from_parent_id(parent, info):
    if parent is None or not parent.get("applianceId"):
        return None
...
def resolve_update_appliance(_, info, applianceId, name=None, public=None):
    # TODO: Maybe make the appliance names unique in the db
...
def resolve_delete_appliance(_, info, applianceId):
