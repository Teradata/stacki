...
ifneq ($(RPM.PACKAGE),)
RPM.NAME ?= $(NAME)-$(RPM.PACKAGE)-$(VERSION)-$(RELEASE).$(RPM.ARCH).deb
else
RPM.NAME ?= $(NAME)-$(VERSION)-$(RELEASE).$(RPM.ARCH).deb
endif
...
ifeq ($(RPM.STRATEGY),source)
	...
	@if [ ! -d $(REDHAT.RPMS)/$(RPM.ARCH) ]; then    mkdir -p $(REDHAT.RPMS)/$(RPM.ARCH); fi
	@echo
	@echo ::: building rpm from source :::
	@echo
	@fpm -f -s tar -t deb --rpm-os linux --deb-no-default-config-files \
		--name $(NAME) --version=$(VERSION) \
		-p $(REDHAT.RPMS)/$(RPM.ARCH)/$(RPM.NAME) \
		$(REDHAT.SOURCES)/$(TARBALL.GZ)
	@echo
	@echo ::: done :::
	@echo
ifndef __ROLLS_MK
...
ifeq ($(RPM.STRATEGY),tar.rpm)
	...
	@echo
	fpm -f -s tar -t deb --rpm-os linux --name $(NAME) --version=$(VERSION); \
	@echo
	@echo ::: done :::
	@echo
	@echo
