...
class ParallelFormat(threading.Thread):
	...
	def run(self):
		tries = 0
			...
			try:
				self.action.execute(intf=self.intf)
...
class DeviceTree(object):
	...
	# Execute an action
        ...
        # 
        # don't parallel format on frontends
        # 
        if 'build' in args:
            parallel_format = 0
	...
	# Execute disk format in parallel
        for action in other_actions:
            ...
            if not dryRun:
                ...
                # this loop
                # 
                if parallel_format and action.isFormat() and \
                        action.isCreate() and action.device.type == "partition":

                    log.info("STACKI:parallel format action: %s" % action)
                    p = ParallelFormat(action, self.intf)
                    threads.append(p)
                    disks.append(action.device.name)
                    continue
                # STACKI
        ...
        if parallel_format and len(threads) > 0:
            ...
            w = self.intf.waitWindow(_("StackIQ Parallel Partition Format"),
                 "Formatting the following partitions in parallel:\n" +
                 "%s" % ', '.join(disks))
    ...
    def addUdevMDDevice(self, info):
        ...
        if device is None:
            ...
            if device:
                ...
                # 
                # we've seen a case where the same software RAID is mapped
                # twice on a host
                # 
                log.info("MD RAID device %s already in "
                                      "devicetree as %s" % (name, device.name))
		return
                # STACKI
